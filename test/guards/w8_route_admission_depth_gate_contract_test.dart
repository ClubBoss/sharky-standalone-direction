...
void main() {
  ...
  const w8RouteEntryPacks = <String>{
  ...
  test('W7 completion can route to W8 when W8 is incomplete', () async {
  ...
  test('active W8 pack state resumes W8 under admitted stale policy', () async {
    for (final activePack in w8RouteEntryPacks) {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboardingCompleted': true,
        'intake_completed_v1': true,
  ...
  test('W8 completion does not open W9 without separate admission', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'onboardingCompleted': true,
      'intake_completed_v1': true,
      'intake_profile_v1':
  ...
  test('stale active W9-W10 pack state remains blocked', () async {
    for (final activePack in w9W10LockedPacks) {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboardingCompleted': true,
        'intake_completed_v1': true,
        'spine_campaign_active_pack_id_v1': activePack,
      ...
      expect(nextPack, isNot(activePack), reason: activePack);
      expect(
        nextPack,
