...
void main() {
  ...
  testWidgets('world9 entry remains actionable on small portrait', (
    tester,
  ) async {
    addTearDown(() {
    ...
    final world9Entry = find.byKey(const Key('world_campaign_open_9'));
    ...
    expect(
      world9Entry.evaluate().isNotEmpty ||
    ...
    } else if (world9Entry.evaluate().isNotEmpty) {
      final entryRect = tester.getRect(world9Entry);
      final logicalHeight =
          tester.view.physicalSize.height / tester.view.devicePixelRatio;
  ...
  testWidgets('world9 routing selects world9 as the next deterministic pack', (
    tester,
  ) async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      'onboardingCompleted': true,
    ...
    expect(expectedPackId, 'world9_spine_campaign_v1');
    ...
    expect(find.textContaining('world9_spine_campaign_v1'), findsNothing);
    expect(tester.takeException(), isNull);
  });
}
