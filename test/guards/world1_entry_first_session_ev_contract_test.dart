...
void main() {
  test(
    'world1 entry keeps table-literacy seat-first and graduates later packs into action decisions',
    () {
  ...
  test(
    'world1 entry action pack now ships a clean raise-call-fold progression',
    () {
      ...
      expect(pack.map((step) => step.heroSeatId).toList(), <String?>[
        'BTN',
        'BB',
        'SB',
      ]);
  ...
  test(
    'world1 entry street-flow pack now proves flop-turn-river contrast directly',
    () {
      final pack = world1MicroTaskPackFor('world1_act0_street_flow');

      expect(pack, hasLength(3));
      expect(pack.map((step) => step.street).toList(), <MicroTaskStreetV1?>[
