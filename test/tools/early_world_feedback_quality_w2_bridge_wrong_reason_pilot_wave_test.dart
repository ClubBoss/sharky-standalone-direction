    ...
    () {
      ...
      for (final entry in expectations.entries) {
        ...
        expect(
          genericIncorrect,
          isNotNull,
          reason: '${entry.key} should keep generic fallback feedback_incorrect_v1.',
        );
        ...
        for (final fieldEntry in entry.value.entries) {
          ...
          expect(
            value,
            isNotNull,
            reason: '${entry.key} should author a wrong-reason message for $actionId.',
          );
          expect(
            value,
            isNot(genericIncorrect),
            reason: '${entry.key} $actionId wrong-reason text should not duplicate generic fallback.',
          );
          for (final snippet in fieldEntry.value) {
            expect(
              value,
              contains(snippet),
              reason: '${entry.key} $actionId wrong-reason text should stay action-specific and scenario-first.',
            );
          }
        }
      }
