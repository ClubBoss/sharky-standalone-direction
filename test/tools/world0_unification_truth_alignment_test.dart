...
void main() {
  test(
    ...
    final routeInventory = buildWorldRouteOwnershipInventoryReportV1(world: 0);
    expect(routeInventory.status, WorldRouteOwnershipInventoryStatusV1.executable);
    ...
    final unificationRow =
        (snapshot['unification_matrix'] as List<Object?>)
            .whereType<Map>()
    ...
    expect(
      ...
      contains('Shared/local ownership is explicit for representative World0 routes'),
    );
    expect(worldRow['visual_family_status'], 'shared');
    expect(worldRow['active_runner_families'], <String>['session_drill_surface']);

    final block =
        (worldRow['blocks'] as List<Object?>)
            .whereType<Map>()
            .map(Map<String, Object?>.from)
