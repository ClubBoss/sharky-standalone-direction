...
void main() {
  test(
    'world8 generic-template wave stays poker-reasoned in w8.s03-w8.s09',
    () {
      ...
      final admittedPrefixes = admittedSessions
          .map(
            (sessionId) =>
                'content/worlds/world8/v1/sessions/$sessionId/drills/',
          )
      ...
      expect(
        ...
        isEmpty,
        reason:
            'w8.s03-w8.s09 should no longer emit generic_template_feedback.',
      );
      ...
      for (final sessionId in admittedSessions) {
        final sessionDir = Directory(
          '$repoRoot/content/worlds/world8/v1/sessions/$sessionId/drills',
        );
        final files =
            sessionDir
                .listSync()
        ...
        expect(
          gradedFiles,
          hasLength(8),
          reason: '$sessionId should keep the admitted 8 graded drills.',
        );

        for (final file in gradedFiles) {
          final json =
              jsonDecode(file.readAsStringSync()) as Map<String, Object?>;
          ...
          switch (kind) {
            ...
            case 'hole_cards_tap':
              ...
              if (cardId != null && cardId.isNotEmpty) {
                ...
                final slotLabel = cardSlot == 'p0'
                    ? 'left hole card'
                    : 'right hole card';
                expect(
                  feedback.contains(slotLabel),
                  isTrue,
                  reason:
                      '${file.path} should name the correct hole-card anchor.',
                );
