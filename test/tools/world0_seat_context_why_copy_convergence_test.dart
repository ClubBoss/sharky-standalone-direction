...
void main() {
  ...
  test(
    'World 0 focus seat-tap why_v1 copy no longer uses the generic seat-context template',
    () {
      ...
      expect(
        File(
          'content/worlds/world0/v1/sessions/w0.s08/drills/d.find_bb_focus.json',
        ).readAsStringSync().contains('big blind seat'),
        isTrue,
      );
      expect(
        File(
          'content/worlds/world0/v1/sessions/w0.s10/drills/d.find_btn_focus.json',
        ).readAsStringSync().contains('final World 0 checkpoint set'),
        isTrue,
      );
    },
  );
  ...
  test(
    'validator no longer reports generic World 0 seat-context why_v1 failures',
    () async {
