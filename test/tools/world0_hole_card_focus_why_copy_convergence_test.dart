...
void main() {
  ...
  test(
    'World 0 mixed-focus hole-card why_v1 copy no longer uses the generic template',
    () {
      ...
      expect(
        File(
          'content/worlds/world0/v1/sessions/w0.s08/drills/d.tap_hole_left_focus.json',
        ).readAsStringSync().contains('big blind side'),
        isTrue,
      );
      expect(
        File(
          'content/worlds/world0/v1/sessions/w0.s09/drills/d.tap_hole_right_focus.json',
        ).readAsStringSync().contains('small blind side'),
        isTrue,
      );
    },
  );
  ...
  test(
    'validator no longer reports generic World 0 mixed-focus hole-card why_v1 failures',
    () async {
      ...
      expect(
        combined.contains(
          'world0_drill_why_generic_hole_card_focus_copy_leak_v1',
        ),
        isFalse,
        reason: combined,
      );
