...
void main() {
  test('beta ship runbook stays bounded under the final-readiness control plane', () {
    ...
    expect(
      runbook,
      contains('it is not the final-product launch authority'),
    );
    expect(runbook, isNot(contains('85/100')));
    expect(
      runbook,
      contains('dart run tools/canonical_early_path_correctness_audit_v1.dart'),
    );

    expect(
      readiness,
      contains('Core Product Readiness'),
    );
    expect(
      reset,
    ...
    expect(
      ladder,
      contains('Current Active Block: `Main-First Final-Readiness SSOT / Authority Migration`'),
    );
