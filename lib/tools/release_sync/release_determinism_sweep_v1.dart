Map<String, Object> buildReleaseDeterminismSweepV1({
  ...
  const fallback = <String, Object>{
    'release_determinism_sweep_v1': false,
    'deterministic_ok': false,
    'issues': <String>['release_determinism_sweep_safe_fallback'],
    'drivers': <String>['release_determinism_sweep_safe_fallback'],
  ...
  for (final key in keys.toList()..sort()) {
    ...
    if (first is Map && second is Map) {
      ...
      if (orderedFirst.toString() != orderedSecond.toString()) {
        issues.add('${key}_nondeterministic');
      }
    }
    drivers.add(key);
  }
  ...
  final deterministicOk = issues.isEmpty;

  return <String, Object>{
    'release_determinism_sweep_v1': true,
