class TableFinalVisualFusionV4 {
  const TableFinalVisualFusionV4(
    this.tableRenderSurfaceV4Map,
    this.tableVisualSnapshotV4Map,
    this.unifiedRenderBundleV1Map,
    this.tableRenderContextV1Map,
    this.tableVisualCompositeSealV1Map,
  );

  final Object tableRenderSurfaceV4Map;
  final Object tableVisualSnapshotV4Map;
  final Object unifiedRenderBundleV1Map;
  final Object tableRenderContextV1Map;
  final Object tableVisualCompositeSealV1Map;

  Map<String, Object> asReadOnlyMap() {
    Map<String, Object> check(Object v) => <String, Object>{
      'exists': v != null,
      'is_map': v is Map,
      'non_empty': v is Map && v.isNotEmpty,
    };
    final Map<String, Map<String, Object>> fusion =
        <String, Map<String, Object>>{
          'render_surface_v4': check(tableRenderSurfaceV4Map),
          'visual_snapshot_v4': check(tableVisualSnapshotV4Map),
          'unified_render_bundle': check(unifiedRenderBundleV1Map),
          'render_context': check(tableRenderContextV1Map),
          'visual_composite_seal': check(tableVisualCompositeSealV1Map),
        };
    final List<String> missing = <String>[];
    fusion.forEach((key, value) {
      final bool exists = value['exists'] == true;
      final bool isMap = value['is_map'] == true;
      final bool nonEmpty = value['non_empty'] == true;
      if (!exists || !isMap || !nonEmpty) missing.add(key);
    });
    final bool ready = missing.isEmpty;
    return <String, Object>{
      'table_final_visual_fusion_v4': <String, Object>{
        'fusion': fusion,
        'missing': missing,
        'fusion_ready': ready,
      },
      'readiness': ready,
    };
  }
}
