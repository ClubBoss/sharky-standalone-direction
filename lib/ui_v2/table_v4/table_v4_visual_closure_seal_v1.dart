class TableV4VisualClosureSealV1 {
  const TableV4VisualClosureSealV1(
    this.tableFinalRenderEnvelopeFusionV4Map,
    this.tableFinalVisualFusionV4Map,
    this.tableRenderSurfaceV4Map,
    this.tableVisualSurfaceV4Map,
    this.tableVisualSealV4Map,
    this.unifiedRenderBundleV1Map,
    this.tableRenderContextV1Map,
  );

  final Object tableFinalRenderEnvelopeFusionV4Map;
  final Object tableFinalVisualFusionV4Map;
  final Object tableRenderSurfaceV4Map;
  final Object tableVisualSurfaceV4Map;
  final Object tableVisualSealV4Map;
  final Object unifiedRenderBundleV1Map;
  final Object tableRenderContextV1Map;

  Map<String, Object> asReadOnlyMap() {
    Map<String, Object> check(Object v) => <String, Object>{
      'exists': v != null,
      'is_map': v is Map,
      'non_empty': v is Map && v.isNotEmpty,
    };
    final Map<String, Map<String, Object>> domains =
        <String, Map<String, Object>>{
          'final_render_envelope_fusion_v4': check(
            tableFinalRenderEnvelopeFusionV4Map,
          ),
          'final_visual_fusion_v4': check(tableFinalVisualFusionV4Map),
          'render_surface_v4': check(tableRenderSurfaceV4Map),
          'visual_surface_v4': check(tableVisualSurfaceV4Map),
          'visual_seal_v4': check(tableVisualSealV4Map),
          'unified_render_bundle_v1': check(unifiedRenderBundleV1Map),
          'render_context_v1': check(tableRenderContextV1Map),
        };
    final List<String> missing = <String>[];
    domains.forEach((key, value) {
      final bool exists = value['exists'] == true;
      final bool isMap = value['is_map'] == true;
      final bool nonEmpty = value['non_empty'] == true;
      if (!exists || !isMap || !nonEmpty) missing.add(key);
    });
    final bool ready = missing.isEmpty;
    return <String, Object>{
      'table_v4_visual_closure_seal_v1': <String, Object>{
        'domains': domains,
        'missing': missing,
        'closure_ready': ready,
      },
      'readiness': ready,
    };
  }
}
