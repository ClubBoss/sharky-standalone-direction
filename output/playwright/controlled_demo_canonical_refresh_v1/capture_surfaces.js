async page => {
  ...
  const dismissAccessibilityGate = async () => {
  ...
  const seedCanonicalEnglish = async () => {
    await page.evaluate(() => {
      localStorage.clear();
      localStorage.setItem('flutter.app_language_code', '"en"');
    });
  };
  ...
  const prepareSurface = async url => {
    ...
    await dismissAccessibilityGate();
