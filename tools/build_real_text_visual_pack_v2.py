...
OUTPUT_ROOT = ROOT / "output" / "design_review" / "real_text_visual_pack_v2"
...
def main() -> None:
    ...
    manifest = {
        "schema": "real_text_visual_pack_v2",
...
def _write_coverage(rows: list[dict[str, object]], commit: str, status: str) -> None:
    lines = [
        "# Real-Text Visual Pack v2 Coverage",
        "",
        f"- Git commit: `{commit}`",
        f"- Git status: `{status}`",
        "- Screens: 19/19",
...
Review the 19-screen real-text evidence pack as a product-surface audit.
...
"""
    (OUTPUT_ROOT / "claude_design_rereview_prompt.md").write_text(
        text, encoding="utf-8"
    )
