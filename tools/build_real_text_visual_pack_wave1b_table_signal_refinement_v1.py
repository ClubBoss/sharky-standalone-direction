#!/usr/bin/env python3
"""Build the local-only Wave 1B table-signal refinement evidence pack."""
...
OUTPUT_ROOT = (
    ...
    / "output"
    / "design_review"
    / "real_text_visual_pack_wave1b_table_signal_refinement_v1"
)
...
def main() -> None:
    ...
    manifest = {
        "schema": "real_text_visual_pack_wave1b_table_signal_refinement_v1",
...
def _write_claude_prompt() -> None:
    text = """# Wave 1B Table Signal Refinement Re-review Prompt

Review the 19-screen real-text evidence pack for Wave 1B: Table Signal
Refinement & First Decision Closure. Compact phone portrait is the quality
target. Tablet is smoke evidence for clipping, overflow, inaccessible actions,
and broken layout only.
...
Primary attention: screens 05-10 and 17-19, with special focus on whether
`06_first_decision` now shows the task and a visible way to act in the compact
first viewport, whether the table clue feels native rather than banner-like,
whether center status/pot is less label-heavy, and whether feedback preserves
the table evidence relationship without adding dense metadata rows.
...
readiness. Static evidence can support visual review only.
...
`docs/_reviews/wave1b_table_signal_refinement_v1.md`
"""
    (OUTPUT_ROOT / "wave1b_table_signal_refinement_rereview_prompt.md").write_text(
        text, encoding="utf-8"
    )
