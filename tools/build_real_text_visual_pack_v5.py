...
OUTPUT_ROOT = ROOT / "output" / "design_review" / "real_text_visual_pack_v5"
...
def main() -> None:
    ...
    manifest = {
        "schema": "real_text_visual_pack_v5",
...
def _write_coverage(rows: list[dict[str, object]], commit: str, status: str) -> None:
    lines = [
        "# Real-Text Visual Pack v5 Coverage",
        "",
        f"- Git commit: `{commit}`",
        f"- Git status: `{status}`",
        "- Screens: 19/19",
...
def _write_claude_prompt() -> None:
    text = """# Claude Design Final Max-10 Re-review Prompt

Review the 19-screen real-text evidence pack as a final phone-first max-quality
design audit for the v5 polish pass. Compact phone portrait is the quality
...
"""
    (OUTPUT_ROOT / "claude_design_final_max10_rereview_prompt.md").write_text(
        text, encoding="utf-8"
    )
