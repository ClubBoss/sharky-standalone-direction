...
readonly contract='tools/contracts/alpha_action_repair_recovery_v1.json'
readonly base_sha="$(git rev-parse --short HEAD)"
readonly bundle="output/review_bundles/alpha_journey_qa_factory_v1_${base_sha}"
...
assert_stale_contract_version_rejected() {
  if python3 tools/validate_alpha_journey_contract_v1.py \
    --contract "$contract" --expected-version 2; then
    echo 'P1 stale journey-contract version unexpectedly passed.' >&2
    return 1
  fi
  echo 'PASS stale journey-contract version rejected clearly.'
}

run_stage 'preflight' git status --short --branch
run_stage 'journey contract' python3 tools/validate_alpha_journey_contract_v1.py --contract "$contract" --expected-version 1
run_stage 'stale contract rejection' assert_stale_contract_version_rejected

cp "$contract" "$bundle/03_journey_contract/"
cp docs/context/ALPHA_JOURNEY_QA_EXECUTION_CAPSULE_TEMPLATE_v1.md "$bundle/02_capsule/"

run_stage 'canonical black-box replay and telemetry trace' flutter test \
  --dart-define=ALPHA_QA_TRACE_PATH="$bundle/07_telemetry/canonical_learn_wrong_repair_recheck_trace.json" \
  test/ui_v2/act0_telemetry_sink_v1_test.dart \
  --plain-name 'canonical Learn entry completes the Action alpha loop with ordered safe telemetry'
...
for viewport in compact tall_phone large_phone; do
  ...
  run_stage "${viewport} Action raster capture" flutter test \
    ...
    --dart-define=ACTION_EVIDENCE_OUTPUT="$bundle/05_phone_evidence/$viewport" \
...
for viewport in compact tall_phone large_phone; do
  cp "$bundle/05_phone_evidence/$viewport/"*contact_sheet.png "$bundle/06_contact_sheets/"
done
...
run_stage 'trace and evidence admission' python3 tools/validate_alpha_journey_contract_v1.py \
  --contract "$contract" \
  --expected-version 1 \
...
printf 'PASS alpha journey QA bundle: %s\nPASS ZIP: %s\n' "$bundle" "$zip_path"
