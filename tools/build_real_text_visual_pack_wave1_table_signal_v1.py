#!/usr/bin/env python3
"""Build the local-only Wave 1 table-signal real-text evidence pack."""
...
OUTPUT_ROOT = (
    ...
    / "output"
    / "design_review"
    / "real_text_visual_pack_wave1_table_signal_v1"
)
...
def main() -> None:
    ...
    manifest = {
        "schema": "real_text_visual_pack_wave1_table_signal_v1",
...
def _write_coverage(rows: list[dict[str, object]], commit: str, status: str) -> None:
    lines = [
        "# Wave 1 Table Signal Real-Text Visual Pack Coverage",
        "",
        f"- Git commit: `{commit}`",
        f"- Git status: `{status}`",
        "- Screens: 19/19",
...
def _write_claude_prompt() -> None:
    text = """# Wave 1 Table Signal First Decision Re-review Prompt

Review the 19-screen real-text evidence pack for Wave 1: Table Signal + First
Decision Actionability. Compact phone portrait is the quality target. Tablet is
...
`06_first_decision` presents a premium first table read, a visible table clue,
clear pot/price hierarchy, intentional Hero/BTN/blind signals, and an obvious
bridge from table clue to feedback.
...
`docs/_reviews/wave1_table_signal_first_decision_v1.md`
"""
    (OUTPUT_ROOT / "wave1_table_signal_first_decision_rereview_prompt.md").write_text(
        text, encoding="utf-8"
    )
