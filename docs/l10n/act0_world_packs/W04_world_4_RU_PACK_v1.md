...
- taskId: frame_open
  ...
  runnerPrompt_en: Folded to BTN with A9s.
  runnerSupport_en: Button still opens the playable suited ace when nobody entered.
  runnerQuestion_en: What is the simple first-in action?
  teachingStep0_title_en: Playable suited ace.
  teachingStep0_body_en: A9s can still open first in when the Button is the acting seat.
  teachingStep0_title_ru: Играбельная рука в поздней позиции.
  teachingStep0_body_ru: KTs на баттоне спокойно играется через открытие, если до тебя никто не вошёл в банк.
  title_ru: Открыть
  runnerPrompt_ru: До баттона все выбросили, у тебя KTs.
...
- taskId: frame_call
  ...
  runnerPrompt_en: CO opened. Hero is BTN with AJs.
  runnerSupport_en: Facing an open, late position can still continue with the stronger broadway hand.
  runnerQuestion_en: What is the simple call-frame action?
  teachingStep0_title_en: Call frame, not open frame.
  teachingStep0_body_en: Once someone opened, AJs does not ask whether to open first in anymore.
  teachingStep0_title_ru: Играбельно и в позиции.
  teachingStep0_body_ru: KQo может просто заколлить открытие, если ты действуешь после CO.
  title_ru: Колл
  runnerPrompt_ru: CO открылся. Ты на баттоне с KQo.
