...
- **Required evidence:** workflow diff; one green run on a real PR; one deliberately induced failure proving the lane fails red (an intentionally broken import and an intentionally failing assertion, both reverted in the same PR); recorded wall-clock runtime; Tier C may remain nonempty only when every path has a valid schema-v2 record, with zero missing/orphan/duplicate records and zero generic placeholders; Tier C/D remain excluded from positive execution; Tier A plus terminal Tier B are fully green.
