...
- World / lesson / task: World 1 / "Poker from Zero" table-read orientation family (hero AK, flop A♥7♣2♦, pot 6 BB, "Step 1/3"). The prior prompt attribution, "What should you read first?", is stale wording from this same family. The active placement-runtime equivalent is "What does this table show?" for `world_1` / `what_poker_is` / `what_poker_is_table_read_transfer` with signal `table_read`.
- Owner (shared layout owner): active route ownership is `Act0ShellPreviewScreenV1` → `placementQuickCheckRunnerV1` → `Act0LessonRunnerShellV1`. The shell owns the compact-answer-list / viewport-pressure system (`compactAnswerListDecision`, `_compactAnswerListUsableHeightBudgetV1`, `_compactAnswerListPhoneMaxShortestSideV1`); `placementQuickCheckRunnerV1` supplies the active table-read question and four-option set, including "Not sure yet".
...
- Validated at HEAD:
- Deterministic evidence: `test/ui_v2/compact_decision_integrity_repair_v1_test.dart` proves the active "What does this table show?" compact check at 375×812: all four options, including "Not sure yet", stay inside the available layout region with a 44 px target, no test-time overflow, and the eligible compact lower-slot path remains bounded.
- Remaining proof: real installed-runtime validation of the active equivalent; final-option tap; resulting feedback transition; bottom safe-area visibility. Status remains `fixed_pending_validation` until that proof exists; do not mark `validated_closed` from deterministic evidence alone.
